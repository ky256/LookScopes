...
兼容 Cursor 和 CodeBuddy 两种 IDE 环境。
"""
...
def setup_logging(project_dir: Path) -> logging.Logger:
    ...
    Returns:
        配置好的 Logger 实例。
    """
    log_path = project_dir / "infra" / "hooks" / "distill_knowledge.log"
...
def load_existing_rules_summary(project_dir: Path) -> str:
    ...
    """
    summaries: list[str] = []
    # 扫描 .cursor/rules/*.mdc（Cursor 扁平结构）
    ...
    if cursor_rules_dir.exists():
        for mdc_file in sorted(cursor_rules_dir.glob("*.mdc")):
            try:
                ...
                pass
    # 扫描 .codebuddy/rules/*/RULE.mdc（CodeBuddy 目录结构）
    codebuddy_rules_dir = project_dir / ".codebuddy" / "rules"
    if codebuddy_rules_dir.exists():
        for rule_mdc in sorted(codebuddy_rules_dir.glob("*/RULE.mdc")):
            rule_name = rule_mdc.parent.name
            # 避免与 Cursor 侧重复（同名规则只保留一条）
            if any(rule_name in s for s in summaries):
                continue
            try:
                text = rule_mdc.read_text(encoding="utf-8")
                first_content_line = _extract_first_content_line(text)
                summaries.append(f"- {rule_name}: {first_content_line}")
            except OSError:
                pass
...
def distill_single(evt: dict, rules_summary: str, target_file_convention: str = "") -> dict | None:
    """对单条事件执行 LLM 蒸馏。

    Args:
        evt: 经验事件 dict。
        rules_summary: 已有规则摘要文本。
        target_file_convention: 目标文件路径约定文本。
    ...
    prompt = SINGLE_DISTILL_PROMPT.format(
        ...
        target_file_convention=target_file_convention,
    )
...
def distill_aggregate(
    events: list[dict], rules_summary: str, target_file_convention: str = ""
) -> list[dict]:
    """对同类型多条事件执行聚合 LLM 蒸馏。
...
def run_distillation(project_dir: Path) -> int:
    ...
    if old_learnings_dir.exists():
        old_events, old_map = ingest_pending_events(old_learnings_dir)
        events.extend(old_events)
        source_map.update(old_map)
    if not events:
    ...
    tfc = get_target_file_convention(detect_ide_prefix(project_dir))

    proposals_created = 0
    processed_event_ids: list[str] = []

    proposals_created += _distill_singles(
        routed["single"], rules_summary, proposals_dir, processed_event_ids, tfc
    )
    proposals_created += _distill_aggregates(
        routed["aggregate"], rules_summary, proposals_dir, processed_event_ids, tfc
    )
...
def _distill_singles(
    ...
    for evt in singles:
        try:
            result = distill_single(evt, rules_summary, target_file_convention)
...
def _distill_aggregates(
    ...
    for top_type, group in aggregates.items():
        try:
            patterns = distill_aggregate(group, rules_summary, target_file_convention)
...
def run_evaluation(project_dir: Path) -> dict:
    ...
    corrections = _load_recent_corrections(
        project_dir / "docs" / "memory" / "learnings"
    )
...
def _build_system_health(
    ...
    rates = _compute_correction_rates(project_dir / "docs" / "memory" / "learnings")
...
def _apply_single_proposal(proposal: dict, project_dir: Path) -> bool:
    ...
    try:
        ...
        if prop_type == "preference:persist":
            ide_prefix = detect_ide_prefix(project_dir)
            if ide_prefix == ".codebuddy":
                identity = project_dir / ".codebuddy" / "rules" / "meta-agent-identity" / "RULE.mdc"
            else:
        ...
        if prop_type == "antipattern:create":
            ide_prefix = detect_ide_prefix(project_dir)
            skill = (project_dir / ide_prefix / "skills"
