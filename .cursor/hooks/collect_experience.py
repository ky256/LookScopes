#!/usr/bin/env python3
"""Cursor hook 元学习经验采集脚本。

由 .cursor/hooks.json 配置触发：
...
EXTRACT_SYSTEM_PROMPT = (
    "你是一个 AI 对话经验分析助手。分析 Cursor IDE 的对话片段，识别用户纠正、"
    "工具失败、偏好表达、规则违反等经验事件。\n"
    "对每个候选片段输出严格的 JSON，不要输出其他内容。"
)

EXTRACT_USER_TEMPLATE = (
    "以下是 Cursor IDE 对话中标记的候选经验片段。请分析并提取结构化经验。\n\n"
    "## 候选片段\n{fragments}\n\n"
...
def setup_logging(project_dir: Path) -> logging.Logger:
    ...
    Returns:
        配置好的 Logger 实例。
    """
    log_path = project_dir / ".cursor" / "hooks" / "collect_experience.log"
...
def build_experience_record(
    ...
    summary_ref = f"docs/summaries/{today}-{session_6}.md"
...
def run_extract(
    ...
    session_6 = session_id[:6]
    today = datetime.now().strftime("%Y-%m-%d")
    output_path = learnings_dir / f"{today}-{session_6}.json"
